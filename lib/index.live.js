...
class DetailedCompactionEngine extends BasicCompactionEngine {
  ...
  async compactIfNeeded(agent, trigger, signal) {
    ...
    try {
      writeFileSync('D:/deepseek/dsh-context-compactor/cc-trigger.log',
        `\n[${new Date().toISOString()}] ${agent.id} trigger=${trigger} total=${beforeTotal} window=${contextWindow ?? '?'} threshold=${thresholdTokens ?? '?'}`,
        { flag: 'a' })
    } catch {}
