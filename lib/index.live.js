...
/** 稳定的 isolate 标签：热重载/重复装载时同标签的 service 槽位会合并去重。 */
const ISOLATE_COMPACTION = Symbol.for('dsh-context-compactor.compaction')
const ISOLATE_PRUNER = Symbol.for('dsh-context-compactor.toolResultPruner')
  ...
  const mountForAgent = (agent) => {
    ...
    try {
      ...
      const compCtx = agentCtx
        .isolate('compaction', ISOLATE_COMPACTION)
        .isolate('toolResultPruner', ISOLATE_PRUNER)
