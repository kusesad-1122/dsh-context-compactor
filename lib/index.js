...
class DetailedCompactionEngine extends BasicCompactionEngine {
  ...
  constructor(ctx, config = {}) {
    const { saveSummaryFile, ...engineFields } = config ?? {}
    // 关闭父类的自动监听，改由本类以 prepend 方式注册。
    super(ctx, { ...engineFields, auto: false })
  ...
  async compactIfNeeded(agent, trigger, signal) {
    ...
    if (trigger === 'pressure' && measurement.totalTokens < thresholdTokens) return null
...
function registerCommands(ctx, cfg, mounted) {
  ...
  const compactHandler = async (invocation) => {
    ...
    // 优先本插件挂到这个 agent 上的详细引擎；其次 agent scope 可见的；最后宿主层。
  ...
  const enhancePromptHandler = async (invocation) => {
    ...
    if (routed !== undefined && routed.provider.length > 0 && routed.model.length > 0) {
      ...
      typeof invocation.agent.options?.provider === 'string' && invocation.agent.options.provider.length > 0
      && typeof invocation.agent.options?.model === 'string' && invocation.agent.options.model.length > 0
    ) {
      target = { provider: invocation.agent.options.provider, model: invocation.agent.options.model }
    }
    if (target === undefined) {
      return { kind: 'error', text: '无法确定当前模型，无法增强提示词。' }
    }
    ...
    if (finish.kind === 'max-tokens') {
      return { kind: 'error', text: '增强结果超出长度限制，请缩短草稿后重试。' }
    }
    const blocks = assembler.blocks().filter((block) => block.type === 'text')
    const output = blocks.map((block) => block.text).join('\n').trim()
    if (output.length === 0) {
      return { kind: 'error', text: '模型没有返回增强结果。' }
    }
...
export function apply(ctx, config) {
  ...
  const mountForAgent = (agent) => {
    ...
    try {
      ...
      // 在自动压缩中先执行，保证「详细总结优先」。
      const compCtx = agentCtx.isolate('compaction').isolate('toolResultPruner')
